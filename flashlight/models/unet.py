...
@new_contract
def up_mode(x):
    return x in ('upconv', 'upsample', 'nearest')
...
class UNetConvBlock(nn.Module):
    ...
    def __init__(self, in_size, out_size, padding, batch_norm, dilation=1):
        super().__init__()
        block = []
...
class UNetUpBlock(nn.Module):
    ...
    def __init__(self, in_size, out_size, up_mode, padding, batch_norm, dilation=1):
        super().__init__()
        if up_mode == 'nearest':
        ...
        elif up_mode == 'trilinear':
            self.up = nn.Sequential(
                nn.Upsample(mode='trilinear', scale_factor=2),
                nn.Conv2d(in_size, out_size, kernel_size=1),
            )
        self.conv_block = UNetConvBlock(
            in_size, out_size, padding, batch_norm, dilation=dilation
        )
